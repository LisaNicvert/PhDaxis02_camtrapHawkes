...
duration_days <- 3 # interaction function support (A in ppstat framework)
...
ggplot(dat) + geom_line(aes(x = t, y = f)) +
  ...
  ylim(c(0, exp(strength)))

# Export plot
ggsave(file.path(figures_path, "function_ppstat.jpeg"),
...
# Export plot
ggsave(file.path(figures_path, "graph_ppstat.jpeg"),
       width = 8, height = 6, dpi = 600, units = "cm", bg = "white")
```
...
plot_observed_rate(lambda_rate, data = Tdf, 
                   ...
                   max_lambda = 10,
                   lwd = .5,
                   ybreaks = seq(0, 10, by = 4),
                   cols = cols)

# Export plot
ggsave(file.path(figures_path, "realization_ppstat.jpeg"),
