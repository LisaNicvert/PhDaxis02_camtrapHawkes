...
source(here("functions/functions_eval.R"))
...
```{r}
# --- Data
data_folder <- here("code/simulation_interevent_times/out/simu_interevents_cluster")
pval_file <-  here("code/simulation_interevent_times/out/pval_precomputed.csv")
...
```{r, fig.width = 10}
# --- Format data
perf_pval <- format_data_perf(perf_pval, level = alpha)

# --- Plot
plot_perf(perf_pval,  
          psize = 1.1,
          xaxis = "Tmax") +
  xlab(paste("Trapping days per camera (× 30 cameras)")) +
...
plot_perf(perf_padj,  
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
# Save figure
ggsave(here("code/simulation_interevent_times/figures/simu_inter_events_padjusted.jpeg"),
...
plot_bias(inference_bias_pval, textsize = 16) +
  xlab("To") +
  ylab("From")
```
...
plot_bias(inference_bias_padj, textsize = 16) +
  xlab("To") +
  ylab("From")
# save figure
ggsave(here("code/simulation_interevent_times/figures/bias_padjusted.jpeg"),
