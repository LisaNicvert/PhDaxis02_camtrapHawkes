# Header #############################################################
# 
# Author: Lisa Nicvert
# Email:  lisa.nicvert@univ-lyon1.fr
# 
# Date: 2022-10-14
#
# Script Description: Allows to import the shapefiles for each reserve


# Import libraries --------------------------------------------------------
library(here)
library(sf)
library(ggsn)
library(dplyr)
library(osmdata)
library(sp)

# Get countries -----------------------------------------------------------

# Define bounding box
SA_dim <- sf::st_bbox(c(xmin = 14, xmax = 38.5, 
                        ymin = -35, ymax = -22),  
                      crs = CRS("+init=epsg:4326"))

# Get countries
q <- opq(bbox =  SA_dim, timeout = 60*2) %>%
  add_osm_feature(key = 'boundary', value = 'administrative') %>%
  add_osm_feature(key = 'admin_level', value = 2)
countries_res <- osmdata_sf(q) # Takes about 2 mins
countries <- countries_res$osm_multipolygons

countries <- countries %>% select(name.en)

# Get reserves -----------------------------------------------------------
bbox_dim <- sf::st_bbox(c(xmin = 26, xmax = 32, 
                          ymin = -28, ymax = -22),  
                        crs = CRS("+init=epsg:4326"))

q <- opq(bbox =  bbox_dim) %>%
  add_osm_feature(key = 'boundary', value = 'protected_area')
parks_res <- osmdata_sf(q)

# Get relevant parks
parks <- parks_res$osm_multipolygons %>% 
  filter(name %in% c("Klaserie Private Nature Reserve",
                     "Kapama Game Reserve",
                     "Thornybush Game Reserve",
                     "Umbabat Nature Reserve",
                     "Kruger National Park",
                     "Madikwe Game Reserve",
                     "Pilanesberg National Park",
                     "Somkhanda Game Reserve",
                     "Balule Nature Reserve",
                     "Venetia Limpopo Nature Reserve"))


# Write to files ----------------------------------------------------------
st_write(parks, here("code/plot_map/shp_data/reserves/reserves.shp"))
st_write(countries, here("code/plot_map/shp_data/southern_africa/southern_africa.shp"))
